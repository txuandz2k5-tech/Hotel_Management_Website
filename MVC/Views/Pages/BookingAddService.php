...
<div class="service-wrapper">
    <div class="service-container">
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 30px;">
            <div>
                ...
                <div class="service-grid" id="serviceGrid">
                    <?php if(!empty($data['services'])): foreach($data['services'] as $service): ?>
        ...
        <div class="booking-info-card" style="margin-top: 20px;">
            <h3 style="color: var(--ocean-blue); border-bottom: 1px solid #444; padding-bottom: 10px;">
                <i class="fas fa-list-ul"></i> Dịch vụ đã đặt
            </h3>
            <div class="used-services-list" style="max-height: 300px; overflow-y: auto;">
                <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                    <thead>
                        <tr style="text-align: left; color: #888; font-size: 0.85rem;">
                            <th style="padding: 10px 5px;">Tên dịch vụ</th>
                            <th style="padding: 10px 5px; text-align: right;">Giá</th>
                            <th style="padding: 10px 5px; text-align: center;">Xóa</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php 
                        $totalService = 0;
                        if (!empty($data['usedServices'])): 
                            foreach ($data['usedServices'] as $used): 
                                // FIX: Sử dụng ThanhTien
                                $price = isset($used['ThanhTien']) ? $used['ThanhTien'] : 0;
                                $totalService += $price;
                                
                                // FIX: Dùng MaDichVuSuDung thay vì id
                                $idToDelete = isset($used['MaDichVuSuDung']) ? $used['MaDichVuSuDung'] : 0;
                        ?>
                        <tr style="border-bottom: 1px solid #333; color: white;">
                            <td style="padding: 10px 5px;"><?= $used['TenDichVu'] ?></td>
                            <td style="padding: 10px 5px; text-align: right; color: #10b981;">
                        ...
                        <?php endforeach; else: ?>
                        <tr>
                            <td colspan="3" style="padding: 20px; text-align: center; color: #666;">Chưa có dịch vụ nào</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            <div style="margin-top: 15px; padding-top: 10px; border-top: 2px dashed #444; display: flex; justify-content: space-between;">
                <span style="color: #888;">Tổng tiền dịch vụ:</span>
                <strong style="color: #f39c12; font-size: 1.1rem;"><?= number_format($totalService) ?> đ</strong>
            </div>
        </div>
    </div>
</div>
...
async function confirmOrder() {
    ...
    for (const [id, service] of selectedServices) {
        try {
            ...
            if (result.success) {
                ...
                console.warn('Không thể thêm dịch vụ ' + id + ': ' + (result.message || 'Lỗi không xác định'));
            }
        } catch (error) {
    ...
    if (successCount > 0) {
        alert(`Đã thêm thành công ${successCount} dịch vụ!`);
        window.location.reload(); 
    } else {
        alert('Không thể thêm dịch vụ. Có thể dịch vụ đã tồn tại hoặc lỗi kết nối.');
    }
}
...
function removeService(id) {
    ...
    if(confirm('Bạn có chắc chắn muốn xóa dịch vụ này khỏi booking?')) {
        fetch(`?controller=ServiceController&action=removeFromBooking&id=${id}`, {
            method: 'POST' 
        })
        .then(response => {
